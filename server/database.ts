...
export const getDb = (customDbPath?: string): Promise<Database> => {
  return new Promise((resolve, reject) => {
    ...
    if (dbPathToUse !== ":memory:") {
      const dbDir = path.dirname(dbPathToUse);
    ...
    const db = new sqlite3.Database(dbPathToUse, (err) => {
      if (err) {
        console.error(
          `Error connecting to database at ${dbPathToUse}:`,
          err.message
        );
        return reject(err);
      }
      ...
      if (dbPathToUse !== ":memory:" && !fs.existsSync(DEFAULT_DB_PATH)) {
        db.serialize(() => {
          db.run(`
...
export const setDbInstanceForTesting = (db: Database | null) => {
  dbInstance = db;
};
